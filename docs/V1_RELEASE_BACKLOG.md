...
| VS-07 | CalendarBoard loading — audit 7; medium; V1 | Empty-looking calendar precedes events; commitment/habit sections expand later despite a fixed calendar height. | Keep the 600px frame and supporting-list space; share the authenticated session snapshot across navigation and mount the actual calendar only after data is resolved. Retain known events during refresh/failure. | Delayed calendar data, true empty and errors; view controls; refresh does not blank events. | Observe Day/Week/Month first load and refresh on mobile/desktop. | CODE COMPLETE |
...
| VS-21 | Feed leaderboard — audit 21; medium; legacy, foundation minute refresh | Every refresh hides known rankings behind Loading; minute/event/self-streak changes cause repeated collapse. | Keep known rows while refreshing; reserve first-load rows; avoid friend refetch solely for local self-streak changes. | Minute/event/self updates, failure, ranking changes and request count assertions. | Leave Feed open across minute boundaries and friend activity; rankings remain visible. | CODE COMPLETE |
| VS-23 | Profile route — audit 23; low; existing/foundation affected | Slow sequential server reads delay navigation without route-specific feedback. | Add profile-shaped loading boundary and parallelize independent authenticated reads; retain server rendering. | Slow profile/summary/count reads, auth redirect, unavailable markers and route transition. | Slow desktop/mobile navigation to Profile with shell continuity. | CODE COMPLETE |
| VS-24 | Feed activity expansion — audit 24; low; legacy | One loading line becomes seven cards and shifts following content. | Reserve compact day-row skeletons in expanded region; retain fresh authorization each open. | Delayed/error/summary-only/detailed histories and repeated open after consent revocation. | Expand on mobile/desktop and revoke sharing between opens. | CODE COMPLETE |
| VS-25 | Friend search and signup availability — audit 25; medium; legacy | Failed search can display stale/current-looking results or no matches; failed username lookup can claim available. | Add read-error outcomes; associate results with resolved query; never infer availability from failed reads. | Lookup failure, query races, clearing/changing query, no matches, taken/available and recovery. | Controlled lookup failures never claim available/no matches or show old results as current. | CODE COMPLETE |
...
### Production observations and Calendar carryover - 2026-09-16

User reports VS-07 remains CODE COMPLETE but **not manually verified**: Calendar briefly appears empty before events arrive in production. The prior frame-only fix is insufficient on route remounts. Repair the Calendar carryover first, before P2 implementation, with session-scoped known data and an unresolved/empty distinction. No production verification is claimed.

Assistant redesign remains P3. Browser and pinch zoom must remain enabled; responsive behavior at 125%, 150% and 200% zoom belongs to Final QA (QA-01 through QA-05), not this implementation scope.

### P2 implementation started - 2026-09-16

VS-16, VS-17, VS-18, VS-19, VS-20, VS-21, VS-23, VS-24 and VS-25 are IN PROGRESS. Calendar carryover focused VS-07/08 tests passed (5), typecheck, lint (one existing warning) and production build passed. Calendar remains CODE COMPLETE, not manually verified. Full combined verification follows P2.

### Calendar carryover and P2 completion - 2026-09-16

**VS-07 carryover and VS-16, VS-17, VS-18, VS-19, VS-20, VS-21, VS-23, VS-24 and VS-25: CODE COMPLETE.** Baseline `0fe855a`; changes remain in the working tree. All unfinished P2 rows were marked IN PROGRESS before implementation and advanced only after passing automated verification. Nothing was deployed or manually verified. The production-observed VS-07 empty flash remains an open production verification gate: the earlier frame-only implementation was insufficient, and the new carryover correction must be checked on the deployed revision.

| ID | Root cause and correction | Regression coverage |
| --- | --- | --- |
| VS-07 | Calendar route mounts recreated null state even though the existing server action already reads tasks, commitments and habits together. A provider inside the user-keyed shell retains that combined snapshot across route mounts. First unresolved entry has a neutral reserved frame; FullCalendar mounts only once data is known. Focus and route-entry revalidation retain known events, including on transient failure. | Added a styled browser regression for first unresolved versus real empty, route remount reuse, failed refresh and focus refresh. Existing Day/Week/Month, frame and task/habit client-navigation tests pass. |
| VS-16 | Client-only preference initialization gated the entire application. Root layout now reads existing authenticated preferences and seeds the saved timezone and initial timestamp. Missing/failed preferences still require initialization; no guessed authenticated timezone is shown or persisted. Unknown/error startup reserves space and retries in place. | Saved timezone, missing/invalid/error seed, anonymous seed, hydration, gated first entry, account changes, obsolete responses and retry. |
| VS-17 | Legacy metrics and charts computed successful-looking zero/empty output from unresolved habits. Consumers use explicit readiness with reserved numeric/chart space and preserve known values on transient failure. Anonymous landing metrics are labeled as requiring sign-in. | Actual Year, Feed and landing components: unresolved data, nonzero values, failed background refresh and genuine zero. Existing Dashboard combined-source readiness regressions pass. |
| VS-18 | Independent shell/page habit hooks fetched separate histories and mutations only updated one consumer. A narrow user-keyed provider inside the clock boundary shares the existing habit state and mutations. Focus refresh deduplicates; read/write ordering retains successful changes; failures retain known state, while authorization loss clears it. | One shared read, route remount reuse, optimistic update/rollback/success propagation, overlapping focus reads, writes during reads, refresh queued with a write, account reset and authorization loss. |
| VS-19 | The nested circle empty branch ignored loading; requests arrived without reserved space. Both empty branches now require resolved friendships, with a reserved first-read request/list region and explicit failure. Known lists remain during reload. | Delayed friendship and profile reads, incoming requests, first failure/retry and genuine empty; prior accepted/outgoing/privacy and removal-race tests remain passing. |
| VS-20 | Summary loading completed against the initial empty event array before events were known. Summaries now depend on event readiness and distinguish an unresolved input from resolved emptiness; known cards remain during revalidation. | Delayed event then summary reads in the actual Feed, hook readiness, known-card refresh/failure retention and true empty; prior partial-failure/consent/revocation tests pass. |
| VS-21 | Self-streak updates refetched friends and loading hid existing rankings. Friend reads now depend on friend identity, minute and feed activity; self values derive locally. Visible rankings remain during refresh. | Self-update request counts, minute refresh, retained known rows and reordered successful rankings; existing failure/authorization tests pass. |
| VS-23 | Sequential Profile reads delayed an otherwise server-rendered route with no shaped fallback. Profile has a route loading boundary, authenticates before parallel profile/summary/count reads, and preserves unavailable markers. | No private reads before authentication, signed-out redirect, all three reads start together, rendered server values and failed-read unavailable markers. |
| VS-24 | A single loading line expanded into seven cards, and late requests could outlive close/reopen. Seven compact reserved rows stabilize expansion; generations ignore obsolete reads and close clears history. Every reopen reauthorizes, with conservative detail removal when consent is revoked. | Seven-row footprint, close/reopen ordering, fresh seven-day reads and summary-only revocation. |
| VS-25 | Failed lookup data was interpreted as no match/available, and stale results were not tied to the current input. Search failures are explicit; only the current resolved query renders matches. Username lookup errors cannot enable signup; obsolete responses are ignored. Existing input debounces remain unchanged. | Search failure, stale results, clearing/query races, no match, username failure/taken/available, disabled signup and recovery. |

The preference seed is request-scoped and read-only. Application routes now render per request instead of being statically prerendered; the manifest remains static. This is the deliberate cost of seeding authenticated timezone preferences. Habit and Calendar snapshots are in memory within existing authenticated React boundaries, never a global cross-user cache or browser-storage copy. No migration, RLS, scheduling, event semantics, OneSignal delivery/authentication, AI redesign, Notifications redesign or zoom restriction was introduced. P3 and the user's design documents were left untouched.
...
- `app/(app)/year/page.tsx`
- `components/calendar/calendar-board.tsx`
- `components/feed/daily-summary-card.tsx`
- `components/friends/add-friend-modal.tsx`
- `components/layout/app-shell.tsx`
- `components/layout/user-clock.tsx`
- `lib/preferences-seed.ts` (new)
- `lib/hooks/use-calendar-data.ts` (new)
- `lib/hooks/use-habits-data.ts`
- `lib/hooks/use-friends-data.ts`
- `lib/hooks/use-feed-data.ts`
- `lib/hooks/use-daily-summaries.ts`
- `lib/hooks/use-leaderboard.ts`
- `lib/hooks/use-activity-history.ts`
- `tests/p1-client.test.mjs`
- `tests/p2-client.test.mjs` (new)
- `tests/p2-server.test.mjs` (new)
- `docs/V1_RELEASE_BACKLOG.md`

| Final validation | Result |
| --- | --- |
| Focused Calendar VS-07/VS-08 tests | 5 passed, including carryover and existing internal navigation |
| `node --test tests/p2-client.test.mjs tests/p2-server.test.mjs` | 26 passed; 0 failed |
| `node --test tests/*.test.mjs` | 178 passed; 0 failed, including P0/P1 and database authorization/privacy regressions |
| `npm.cmd run typecheck` | Passed |
| `npm.cmd run lint` | Passed: 0 errors; one existing `postcss.config.mjs` anonymous-default-export warning |
| `node node_modules/next/dist/bin/next build` | Passed on final application source; request-rendered application routes and static manifest generated |
| `node scripts/ui-build.cjs` and `node scripts/ui-check.cjs` | 20 existing route/viewport checks passed at 375, 390, 430 and 1280px |
| Additional P2 responsive regressions | Pending and settled Feed/Year/Friends at all four widths: 12 route/viewport combinations; no document overflow |
| Final diff/spec/scope review and `git diff --check` | Passed; no P3, migration, authorization weakening, fake events or forced reload added |

New coverage totals 27 tests: one Calendar carryover test plus 23 P2 client/browser tests and three server tests. Browser tests use deferred synthetic transport and real components; database tests use local fixtures. The 375px Calendar screenshot from the existing responsive suite was inspected. These checks do not establish hosted Auth/transport/provider performance or physical-device behavior. An initial new-test fixture issue and a test-helper lint naming issue were corrected; final commands above passed.

#### Exact production verification still required

1. **VS-07/08:** On the deployed revision, populate tasks, timed habits and commitments. At 375/390/430px and desktop, throttle Calendar cold entry and navigate away/back repeatedly through Day/Week/Month. Known events must be visible immediately on repeat navigation and survive slow/failed focus refresh. A genuinely empty account must become empty only after resolution; failed initial reads must be unavailable. Follow task/habit events and browser Back; verify shell/document continuity and modified/new-tab behavior. Record evidence that the original production empty flash is gone before advancing VS-07.
2. **VS-16/17/18:** Cold-load landing/auth/Dashboard/Feed/Year with existing saved timezone, then with a new account needing initialization. Verify no wrong-zone intermediate content, hydration errors or fabricated zero values. Check local midnight/DST boundaries. Complete/reopen/edit/create/delete habits and compare shell/Dashboard/Feed/Year; known values must survive repeat navigation and controlled read failures. Exercise logout, account switch and a slow old-account response; no previous-account data may remain. Confirm request-rendered startup latency in the deployed environment.
3. **VS-19/20/21:** Use accounts with accepted friends, incoming/outgoing requests and no friends. Throttle friendship/profile/event/summary reads separately. Neither Friends nor Feed may briefly claim empty. Leave Feed open across minute boundaries and real friend check-ins; rankings and known summaries must remain visible. Local self-streak changes must not independently refetch every friend's streak.
4. **VS-23/24 and privacy:** Under slow transport navigate to Profile; shell remains interactive, loading is shaped, and final server metrics match the account. Signed-out access redirects. With A/B friends and unrelated C, open/close/reopen seven-day activity; revoke detailed sharing or friendship before reopening and verify fresh server authorization, no old private names and no unrelated access. Observe expansion geometry on phone and desktop.
5. **VS-25:** In signup and Add friend, fail lookup reads using a controlled test path, change/clear/retype input rapidly and resolve old responses late. Failure must never mean available/no matches, old search results must not appear current, signup stays blocked until the current username has a successful availability result, and recovery/taken/no-match behavior remains correct.
6. **Final QA:** Browser and pinch zoom remain enabled. Check 125%, 150% and 200% zoom, Safari/Firefox/Android Chrome, physical devices and the remaining release matrix on the deployed candidate. These gates remain TODO; fixture Chromium responsive checks do not mark them manually verified.

All requested Calendar carryover and unfinished P2 code work is complete. Deployment and the production/manual checks above remain open. Stop after P2; Assistant redesign and all P3 work remain deferred.
