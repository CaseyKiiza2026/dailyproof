...
export async function deliverNotifications() {
  ...
  if (enqueueError) throw new Error("Unable to enqueue due reminders.");
  const { data, error } = await db.rpc("claim_push_notifications");
  if (error) throw new Error("Unable to claim notifications.");
  await Promise.all(
    (data ?? []).map(async (row: DeliveryRow) => {
      let result = "sent",
        failure: string | null = null;
      try {
        const { data: settings, error: settingsError } = await db
          .from("notification_settings")
          .select("enabled,push_alias,nudges,friend_activity")
        ...
        if (
          !settings?.enabled ||
          (row.type === "nudge" && !settings.nudges) ||
          (row.type === "friend_activity" && !settings.friend_activity)
        ) {
          result = "skipped";
        } else {
          if (
          ...
          if (!response.ok || !body.id)
            throw new Error(
              `Push delivery failed (${response.status}); check subscription and OneSignal configuration.`,
            );
        }
      } catch (e) {
        result = "failed";
        failure = e instanceof Error ? e.message : "Push delivery failed.";
      }
      const { error: finishError } = await db.rpc("finish_push_notification", {
        p_id: row.id,
      ...
      if (finishError)
        throw new Error(
          "Unable to record delivery result; the same delivery key will be retried.",
        );
    }),
  );
